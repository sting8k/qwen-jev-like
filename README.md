...
|---|---|---|
| file on disk | 6872 MiB | 9374 MiB |
...
| **GPU total** (weights + KV + state + compute) | **10 423 MiB** | **12 474 MiB** |
...
One unit throughout so the column adds up: 10 423 MiB is 10.9 GB, and 12 474 MiB is 13.1 GB. The gap between the
columns is 2051 MiB, which is 2091 MiB of weights less 40 MiB of compute buffer; KV and recurrent state are
identical on both.

`nvidia-smi` reads higher than the total: it also counts the CUDA context and the desktop session on the same card.
The KV cache scales with context too, reaching 1768 MiB at `ctx/seq=4096`, the shape the scenario benches use.
