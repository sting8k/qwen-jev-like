...
One digest in that set is derived rather than measured, and its README says so
in the first sentence: it is the digest of the file the current builder
produces, reached by transforming the measured corpus rather than by running the
builder. The two files differ in their row ids, 991 of them and 3964 bytes, and
in nothing else: same 300 reports, same 991 rows, same labels.
